...
def output_batch_query(inputPath, outputPath, ontologies_param = None):
    ...
    for term_matches in results_list: # iterating through list of lists; each iteration is list of all matches for a term
        for match in term_matches: # iterating thruh list of dictionaries each representing a match
            record = []
            for key in ALL_KEYS:
...
def resolve_list(listOfTerms, ontologies_param):
    search_results = []
...
def resolve_term(term, ontologies_param):
    ...
    for page in pages:
        for match in page['collection']:
            ...
            time.sleep(.1) # to avoid overloading BioPortal's API; .1s is as low as I've set the delay so there is room for experimentation
...
def term_query_string(term, ontologies):
    search_term = 'q=' + term
